...
# cuadriga ROS2 Package Launch Files

This document provides detailed information about the launch files used in the cuadriga ROS2 package. Below are explanations of the launch files for both the simulator and the real robot, including which nodes are launched and how the lifecycle transitions are handled.

## Configuration Files

- [ctrl_mission.yaml](../config/cuadriga_config.yaml): Configuration for both the simulator and the real robot.
...
The unified launch file [cuadriga_launch.py](./cuadriga_launch.py) is used to start the nodes for either the simulator or the real robot environment based on the specified launch parameters.

- **File:** `launch/cuadriga_launch.py`
...
- `cuadriga_sensors`: Initializes the sensors node.
