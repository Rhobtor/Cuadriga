...
Package of mission control for rover cuadriga. The mission mode is controlled by a Finite State Machine.

## Finite State Machine

A Finite state machine (implemented in the file `cuadriga_FSM.cpp`) manage the [states](</cuadriga_ctl_mission/src/states>)  transitions between different states. These states implement the different functionalities of the robot. In particular:
...
3. **Pathfollowing (State 2)**. In this state, the robot performs a path tracking algorithm using one of the [controllers](</cuadriga_ctl_mission/src/controllers>) included in the package.
...
![image_FSM](./images/cuadriga_diagram.png)

The robot follows the path specified in the topic `*/cuadriga/waypoints*` in order, using the goal GPS position, the robot GPS position, and the global orientation. If the distance to the current goal is less than the one specified in the `*distance_get_goal*`, the current goal would be replaced by the next in the list. Furthermore, to be able to follow complex trajectories, the linear speeds depend on the linear speed as:
