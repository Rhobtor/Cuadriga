...
class ROSclass_topics(Node):
    def __init__(self):
        super().__init__('cuadriga_GUI_topics')
        # Declare parameters with default values
        self.declare_parameter('filtered_gps_topic_name', '/fixposition/navsatfix')
        self.declare_parameter('base_gps_topic_name', '/cuadriga/base/gps/fix')
        self.declare_parameter('rover_gps_topic_name', '/cuadriga/rover/gps/fix')
        self.declare_parameter('imu_head_topic_name', '/imu_head/data')
        self.declare_parameter('look_ahead_topic_name', '/cuadriga/look_ahead_point')
        self.declare_parameter('local_path_topic_name', '/cuadriga/local_trajectory_ll')
...
class ROSclass_srv(Node):
    def __init__(self):
        super().__init__('cuadriga_GUI_srv')
        ...
        self.change_cuadriga_FSM = self.create_client(ChangeMode, change_fsm_mode_srv_name)
        self.get_cuadriga_path = self.create_client(ReturnRobotPath, return_path_service)
    ...
    def get_cuadriga_FSM_mode(self):
        request = GetMode.Request()
        future = self.get_cuadriga_FSM.call_async(request)
        rclpy.spin_until_future_complete(self, future, timeout_sec=1.0)
        if (future.result() == None):
#          print('is notetype')
          return -1
    ...
    def get_cuadriga_path_request(self):
        request = ReturnRobotPath.Request()
        future = self.get_cuadriga_path.call_async(request)
        rclpy.spin_until_future_complete(self, future, timeout_sec=1.0)
        return future.result()
    ...
    def send_FSM_change_request(self,transition):
        ...
        future = self.change_cuadriga_FSM.call_async(request)
        rclpy.spin_until_future_complete(self, future, timeout_sec=1.0)
