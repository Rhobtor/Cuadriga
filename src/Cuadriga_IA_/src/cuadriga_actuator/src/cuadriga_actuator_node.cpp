...
: Node("cuadriga_actuator_node")
{
  ...
  declare_parameter("command_mapping", "twist_channels");
  ...
  declare_parameter("angular_mix_gain", 1.0);
  declare_parameter("pwm_limit", 127);
  declare_parameter("allow_reverse_linear", true);
  declare_parameter("invert_turn_direction", false);
  declare_parameter("send_angular_first", false);
  ...
  command_mapping_name_ = get_parameter("command_mapping").as_string();
  ...
  angular_mix_gain_ = get_parameter("angular_mix_gain").as_double();
  ...
  invert_turn_direction_ = get_parameter("invert_turn_direction").as_bool();
  ...
  std::string normalized_mapping = command_mapping_name_;
  std::transform(
    normalized_mapping.begin(), normalized_mapping.end(), normalized_mapping.begin(),
    [](unsigned char character) { return static_cast<char>(std::tolower(character)); });
  if (normalized_mapping == "twist_channels") {
    command_mapping_ = CommandMapping::TwistChannels;
    command_mapping_name_ = "twist_channels";
  } else {
    command_mapping_ = CommandMapping::Differential;
    command_mapping_name_ = "differential";
  }
...
bool CuadrigaActuatorNode::sendMotion(double linear_velocity, double angular_velocity)
{
  ...
  const auto [channel_a_pwm, channel_b_pwm] = computeDrivePwms(linear_velocity, angular_velocity);

  const auto angular_command = encodePwmCommand(channel_a_pwm, 'A', 'a');
  const auto linear_command = encodePwmCommand(channel_b_pwm, 'B', 'b');
...
std::array<int, 2> CuadrigaActuatorNode::computeDrivePwms(
  double linear_velocity,
  double angular_velocity) const
{
  if (command_mapping_ == CommandMapping::TwistChannels) {
    return {
      angularToPwm(angular_velocity, angular_velocity_max_),
      velocityToPwm(linear_velocity, linear_velocity_max_)
    };
  }

  const double max_linear = std::max(linear_velocity_max_, std::numeric_limits<double>::epsilon());
  const double max_angular = std::max(angular_velocity_max_, std::numeric_limits<double>::epsilon());

  const double linear_norm = std::clamp(linear_velocity / max_linear, -1.0, 1.0);
  double turn_norm = std::clamp(angular_velocity / max_angular, -1.0, 1.0) * angular_mix_gain_;
  if (invert_turn_direction_) {
    turn_norm = -turn_norm;
  }

  const double channel_a = std::clamp(linear_norm + turn_norm, -1.0, 1.0);
  const double channel_b = std::clamp(linear_norm - turn_norm, -1.0, 1.0);

  return {
    static_cast<int>(std::lround(channel_a * static_cast<double>(pwm_limit_))),
    static_cast<int>(std::lround(channel_b * static_cast<double>(pwm_limit_)))
  };
}
