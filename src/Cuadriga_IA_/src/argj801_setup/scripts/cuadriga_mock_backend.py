...
class CuadrigaMockBackend(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('command_mapping', 'differential')
        ...
        self.command_mapping = str(self.get_parameter('command_mapping').value).lower()
        ...
        self._channel_a_command = 0
        self._channel_b_command = 0
    ...
    def _on_serial_write(self, msg: UInt8MultiArray) -> None:
        ...
        if self.command_mapping == 'twist_channels':
            if channel == 'linear':
            ...
            if channel == 'channel_a':
                self._channel_a_command = signed_value
            elif channel == 'channel_b':
                self._channel_b_command = signed_value
            self.linear_command, self.angular_command = self._mix_from_tracks(
                self._channel_a_command,
                self._channel_b_command,
            )
    ...
    def _decode_command(self, payload: bytes) -> Optional[tuple[str, int]]:
        ...
        if prefix in {'A', 'a'}:
            return ('angular', signed_value) if self.command_mapping == 'twist_channels' else ('channel_a', signed_value)
        if prefix in {'B', 'b'}:
            return ('linear', signed_value) if self.command_mapping == 'twist_channels' else ('channel_b', signed_value)
        return None

    def _mix_from_tracks(self, channel_a_pwm: int, channel_b_pwm: int) -> tuple[float, float]:
        channel_a = self._scale_to_speed(channel_a_pwm, self.max_linear_speed)
        channel_b = self._scale_to_speed(channel_b_pwm, self.max_linear_speed)
        linear = 0.5 * (channel_a + channel_b)
        angular = ((channel_a - channel_b) / max(self.max_linear_speed, 1e-6)) * (0.5 * self.max_yaw_rate)
